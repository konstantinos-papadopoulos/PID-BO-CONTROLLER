...
%          while Tp2 <= 1
            ...
            for i = 1:outerloop
                p1(i) = tp1 + tp2 + tp3(i)               ;
                p2(i) = tp1*tp2 + tp1*tp3(i) + tp2*tp3(i);
                p3(i) = tp1 * tp2 * tp3(i)               ;
...
%             ti_3(i) = 2*kp*(q1(i) - z1 - x3(i));
            ...
            figure(1)
%             subplot(2,1,1)
%             plot(tp3,tn,'r',tp3,tv,'k',tp3,D,'c')
%             grid
%             hold on
%             subplot(2,1,2)
            str = i_nit(1)        ;
...
%             plot(tp3(str:endpl),x(str:endpl),'r',tp3(str:endpl),y(str:endpl),'b',tp3,D,'g','LineWidth',2)
            plot(tp3(1:str),tn(1:str),'r--',tp3(1:str),tv(1:str),'k',tp3,D,'g-.','LineWidth',2)
%             plot(tp3,ti_3,'r','LineWidth',2)
            current_line_up = zeros(outerloop,1)  ;
            grid
            hold on
            plot(tp3(endpl:outerloop),tn(endpl:outerloop),'r--',tp3(endpl:outerloop),tv(endpl:outerloop),'k',tp3,D,'g-.','LineWidth',2)
            hold on          
        end
...
% end
            line(tp3,zeroLine,'Color','k','LineStyle','--','LineWidth',2)
