...
function [Fcl_MO Fcl_optimal Gp stepinformation ti_MO x_MO y_MO optimalControllerData] = autotune_param_typeII_calculateOvsNoTszpid(plant_loc)
load fismat_y_estimator_pid_1-2
...
tn_MO = tn              ;
x_MO  = tn_MO           ;
fuzzy_y(1) = x_MO_pi_loc;
fuzzy_y(2) = x_MO       ;

y = evalfis([fuzzy_y(1) fuzzy_y(2)],fismat_y_estimator_pid);
% ti_MO = sqrt((1/2)*kp*kh*(tn_MO^2 + q1^2 - 2*q2));
% tsz = 1.941025;
tsz = 0;
ti_MO = sqrt((1/2)*kp*kh*(tn_MO^2 - 2*y + tsz));
